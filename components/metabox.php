...
// Add the Meta Box
function gravixar_portfolio_add_custom_meta_box() {
    add_meta_box(
        'custom_meta_box', // $id
        'Gravixar Portfolio Fields', // $title
        'gravixar_portfolio_show_custom_meta_box', // $callback
        'gravixar_portfolio', // $page
        'normal', // $context
        'high'); // $priority
}
add_action('add_meta_boxes', 'gravixar_portfolio_add_custom_meta_box');

// Field Array
$prefix = 'gravixar_portfolio_';
...
// The Callback
function gravixar_portfolio_show_custom_meta_box($object) {
        ...
        foreach ($custom_meta_fields as $field) {
                ...
                switch($field['type']) {
                        ...
                        $close_button = null;
                        if ($meta) {
                                $close_button = '<span class="gravixar_mobileportfolio_close"></span>';
                        }
                        echo '<input id="gravixar_portfolio_mobileimage" type="hidden" name="gravixar_portfolio_mobileimage" value="' . esc_attr($meta) . '" />
                        <div class="gravixar_portfolio_mobileimage_container">' . $close_button . '<img id="gravixar_portfolio_mobileimage_src" src="' . wp_get_attachment_thumb_url(gravixar_portfolio_get_image_id($meta)) . '"></div>
                        <input id="gravixar_portfolio_mobileimage_button" type="button" value="Add Image" />';
                        break;
                        case 'gallery':
                        $meta_html = null;
                        if ($meta) {
